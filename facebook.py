...
class GraphAPI(object):
    ...
    def request(self, path, args=None, post_args=None):
        ...
        """
        #print ("&&&&& in request.  Args are: ",args)
        args = args or {}
        ...
        try:
            #urlString='https://graph.facebook.com/' + path+'?'+urllib.urlencode(args)
    ...
    def fetch_next_page(self,page_url):
        """ Fetches the next page of paged data.  It takes the page_url that
        facebook returns when there is paged data that has not yet been processed.
        
        """
        
        try:
        
            file = urllib2.urlopen(page_url,timeout=self.timeout)
        except urllib2.HTTPError, e:
            response = _parse_json(e.read())
            raise GraphAPIError(response)
        except TypeError:
